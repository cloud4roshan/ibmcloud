/*
#---------------------------------------------------------------------------------------------
# SSH Key for LON04
#---------------------------------------------------------------------------------------------
resource "ibm_pi_key" "lon04-ssh-key" {
  pi_key_name          = "lon04-ssh-key"
  pi_ssh_key           = "ssh-rsa  cloud4roshan@vm-jumphost"
   pi_cloud_instance_id = resource.ibm_pi_workspace.pws01-prd-lon04.id
}
#---------------------------------------------------------------------------------------------

#---------------------------------------------------------------------------------------------
# SSH Key for LON06
#---------------------------------------------------------------------------------------------
resource "ibm_pi_key" "lon06-ssh-key" {
  pi_key_name          = "lon06-ssh-key"
  pi_ssh_key           = "ssh-rsa  cloud4roshan@vm-jumphost"
   pi_cloud_instance_id = resource.ibm_pi_workspace.pws01-prd-lon06.id
}
#---------------------------------------------------------------------------------------------

#---------------------------------------------------------------------------------------------
# SSH Key for WDC06 
#---------------------------------------------------------------------------------------------
resource "ibm_pi_key" "wdc06-ssh-key" {
  pi_key_name          = "wdc06-ssh-key"
  pi_ssh_key           = "ssh-rsa  cloud4roshan@vm-jumphost"
   pi_cloud_instance_id = resource.ibm_pi_workspace.pws01-prd-wdc06.id
}
#---------------------------------------------------------------------------------------------

#---------------------------------------------------------------------------------------------
# SSH Key for WDC07 
#---------------------------------------------------------------------------------------------
resource "ibm_pi_key" "lon04-ssh-key" {
  pi_key_name          = "wdc07-ssh-key"
  pi_ssh_key           = "ssh-rsa  cloud4roshan@vm-jumphost"
   pi_cloud_instance_id = resource.ibm_pi_workspace.pws01-prd-wdc07.id
}
#---------------------------------------------------------------------------------------------
*/
